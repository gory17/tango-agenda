...
class FabienFbApi{
	...
	function __construct(){
		$this->accessToken= "EAACwhIfUxt8BANZBfGgLDnZCxMu6tFthvLwpkXVZAYc6uL80Uc26VeUPlwTvdjZAbTD2B50uZAtC49D5fCGL7er0p3ySY83uBi1v00C17lAxfZCtuiGBhiyaCofQtdRUhCW8ZAKGpnCZCVWV6EZASS8Mp3l18RsOcpTVk3yQjefURQwZDZD";
	...
	public function listFbEvents($searchReq,$userTokenFB,$mode="requete",$banned){
			...
			try {
			  // Returns a `Facebook\FacebookResponse` object
				if($mode=="requete"){
					...
					$response = $fb->get("$searchReq/events",$userTokenFB);
					//print_r($response);
				}
		...
		foreach($graphEdge as $graphNode){
				...
				if(isset($graphNode['start_time']) and !in_array($graphNode['id'],$banned)){
					$dateNode=$graphNode['start_time'];
		...
		*/
		...
		if($mode=="groups"){
				try {
					$responseFeed = @$fb->get("$searchReq/feed?limit=5",$userTokenFB);
				} catch(Facebook\Exceptions\FacebookResponseException $e) {
					return 'Graph returned an error: ' . $e->getMessage();
					exit;
				} catch(Facebook\Exceptions\FacebookSDKException $e) {
				...
				foreach($graphFeed as $feed){
					...
					*/
					...
					if($timeChecker>new \DateTime("now -3 days") and !in_array($feed['id'],$banned)){
						try {
							$responseAttachement = $fb->get($feed['id']."/attachments",$userTokenFB);
						...
						foreach($graphEdgeAttachement as $graphNode){
							...
							if($graphNode['type']=='event'){
								...
								try {
									...
									$responseEventFeed=$fb->get($graphNode['target']['id']."?fields=id,start_time",$userTokenFB);
									}else{
