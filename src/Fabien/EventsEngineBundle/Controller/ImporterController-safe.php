        ...
        if($response){
          ...
          if(!$targetCity){
            ...
            foreach($usedCities as $usedCity){
                ...
                if(strpos($response['city'],$usedCity->getTitle())){
                  $event->setCity($usedCity);
                  break;
                }
                if(strpos($response['city'],$usedCity->getTitleBis())){
                  $event->setCity($usedCity);
                  break;
                }
                if(strpos($response['place'],$usedCity->getTitle())){
                  $event->setCity($usedCity);
                  break;
                }
                if(strpos($response['place'],$usedCity->getTitleBis())){
                  $event->setCity($usedCity);
                  break;
                }
          ...
          if(!$targetCity)$targetCity=$this->getGeocodeCity($response);
        ...
        if($fbEvent->getTentative()>2){
          ...
          if($event){
            if($this->existFB($event->getUrlFb())!=true){
              $cptSave++;
              if($event->getTitle()!="" and strpos($event->getTitle(),"atchata")==false and strpos($event->getTitle(),"alsa")==false and strpos($event->getTitle(),"achata")==false and strpos($event->getTitle(),"kiz")==false and strpos($event->getTitle(),"izumba")==false and strpos($event->getTitle(),"izomba")==false){
    ...
    foreach($arrayTexte as $item){
      if(strlen($item)>=4){
        ...
        switch ($item){
          ...
          default :
            //$idGuessType=11;
            if(strpos($texte,"longa"))$idGuessType=1;
            break;
        }
