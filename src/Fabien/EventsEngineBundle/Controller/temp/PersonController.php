...
class PersonController extends Controller
{
    ...
    public function listMaestroAction(){
      ...
      return $this->render('FabienEventsEngineBundle:person:listmaestro.html.twig',array("listmaestros"=>$listmaestros));
      ...
      foreach($maestros as $maestro){
        ...
        $search3=$this->clarifyText($maestro->getPrenom().' "'.$maestro->getSurnom().'" '.$maestro->getNom());
          ...
          foreach($listDatesBigEvents as $dateBigEvent){
			...
			if($dateBigEvent->getEvent()->getTypeEvent()->getId() != 1 and $dateBigEvent->getEvent()->getTypeEvent()->getId() != 2){
				if((strpos($this->clarifyText($dateBigEvent->getEvent()->getDescription()),$this->clarifyText($maestro->getNom()))!==false) and strpos($this->clarifyText($dateBigEvent->getEvent()->getTitle()),$this->clarifyText($maestro->getPrenom()))!==false)$link=true;
			}
			
    
            foreach($dateBigEvent->getEvent()->getPersons() as $personInEvent){
              if($personInEvent==$maestro)$link=false;
            }
