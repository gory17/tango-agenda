...
class PersonController extends Controller
{
    ...
    public function linkMaestrosAction(){
      ...
      foreach($maestros as $maestro){
        ...
        $search1=$this->clarifyText($maestro->getPrenom()." ".$maestro->getNom());
        $search2=$this->clarifyText($maestro->getPrenom()." ".$maestro->getSurnom()." ".$maestro->getNom());
        $search3=$this->clarifyText($maestro->getPrenom().' "'.$maestro->getSurnom().'" '.$maestro->getNom());
		$search4=$this->clarifyText($maestro->getPrenom()." & ".$maestro->getPartner()->getPrenom());
		$search5=$this->clarifyText($maestro->getPrenom()." y ".$maestro->getPartner()->getPrenom());
          ...
          foreach($listDatesBigEvents as $dateBigEvent){
			...
			if($dateBigEvent->getEvent()->getTypeEvent()->getId() != 1 and $dateBigEvent->getEvent()->getTypeEvent()->getId() != 2 and $dateBigEvent->getEvent()->getTypeEvent()->getId() != 10){
				...
				if((strpos($this->clarifyText($dateBigEvent->getEvent()->getTitle()),$this->clarifyText($maestro->getNom()))!==false) and strpos($this->clarifyText($dateBigEvent->getEvent()->getTitle()),$this->clarifyText($maestro->getPrenom()))!==false){
				...
				if(strpos($this->clarifyText($dateBigEvent->getEvent()->getDescription()),$this->clarifyText($maestro->getNom()))!==false and strpos($this->clarifyText($dateBigEvent->getEvent()->getDescription()),$this->clarifyText($maestro->getPrenom()))!==false){
					$link=true;
					echo $this->clarifyText($maestro->getPrenom())." ".$this->clarifyText($maestro->getNom())." - ".$this->clarifyText($dateBigEvent->getEvent()->getTitle())."<br>";
				}
			
			
				if(strpos($this->clarifyText($dateBigEvent->getEvent()->getTitle()),$this->clarifyText($maestro->getPrenom())." & ".$this->clarifyText($maestro->getPartner()->getPrenom())) or strpos($this->clarifyText($dateBigEvent->getEvent()->getTitle()),$this->clarifyText($maestro->getPrenom())." y ".$this->clarifyText($maestro->getPartner()->getPrenom()))){
					$link=true;
					echo $this->clarifyText($maestro->getPrenom())." ".$this->clarifyText($maestro->getNom())." - ".$this->clarifyText($dateBigEvent->getEvent()->getTitle())."<br>";
				}
