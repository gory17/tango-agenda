...
class CountryController extends Controller
{
  ...
  public function listAction()
  {
    ...
    $countEvents= $this
      ->getDoctrine()
      ->getManager()->getRepository('FabienEventsEngineBundle:Event')->countTotalEvents();
    ...
    return $this->render("FabienEventsEngineBundle:Country:list.html.twig",array("CountryList"=>$CountryList,"countEvents"=>$countEvents));
  }
  ...
  public function listCitiesAction(Country $country,$slug){
    ...
    ->getDatesQuery("",array(3,4,5,10),"","",$country)
    ;

    $start_date=new \DateTime("now");
    $end_date=new \DateTime("now 360 Days");


    $datesListImportant=$this->getDoctrine()
    ->getManager()
    ->getRepository("FabienEventsEngineBundle:Date")
    ->getDatesQuery("",array(3,4,5,12,10),"all","",$country,'','','','',1,0,$start_date,$end_date);

    $mostActiveCityList=$repositoryCity->getCountryCities($country->getSlug(),"cptevt","DESC",21);
    $cityList=$repositoryCity->getCountryCities($country->getSlug());

    return $this->render("FabienEventsEngineBundle:Country:countryView.html.twig",array("country"=>$country,"mostActiveCityList"=>$mostActiveCityList,"cityList"=>$cityList,"lastEvents"=>$lastEvents,"start_date"=>$start_date,"end_date"=>$end_date,"datesListImportant"=>$datesListImportant));
  }
