...
class DateRepository extends \Doctrine\ORM\EntityRepository
{
  ...
  public function getDatesQuery($city=false,$type=false,$period="week",$region=false,$country=false,$fieldOrder="d.start",$order="ASC",$limit=0,$offset=0,$published=1,$valorisation=0,$startDate="",$endDate="",$person=""){
      ...
      if($person!=false){
        $operation->join("e.persons","persons")
      ...
      if($city!=false){
		...
		*/
        ...
        $operation->join("e.city","city")
                  ->andWhere('((city.lat BETWEEN :minlat AND :maxlat) and (city.lng BETWEEN :minlng AND :maxlng)) or (city=:city) ')
                      ->setParameter('minlat',$minlat)
                      ->setParameter('maxlat',$maxlat)
                      ->setParameter('minlng',$minlng)
                      ->setParameter('maxlng',$maxlng)
